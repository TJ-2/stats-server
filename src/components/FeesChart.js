...
export default function FeesChart(props) {
  ...
  const csvFields = [
    ...
    { key: 'mint', name: 'Mint OLP' },
    { key: 'burn', name: 'Burn OLP' },
    ...
    >
      <ResponsiveContainer width="100%" height={chartHeight}>
        <ComposedChart data={formattedData} syncId="syncA">
          ...
          <Bar unit={viewSettings.itemsUnit} isAnimationActive={false} type="monotone" dataKey="mint" stackId="a" name="Mint OLP" fill={COLORS[1]} />
          <Bar unit={viewSettings.itemsUnit} isAnimationActive={false} type="monotone" dataKey="burn" stackId="a" name="Burn OLP" fill={COLORS[2]} />
      ...
      <div className="chart-description">
        Collected fees. USD value is calculated with token price at the moment of swap, trade, minting or redeeming OLP
      </div>
    </ChartWrapper>
  )
}